...
-spec start_link(Tab :: ets:tab()) -> {ok, pid()} |
...
-spec handle_call(Msg, From, State) -> reply(Reply, NewState) |
                                       noreply(NewState) |
                                       {stop, Reason, Reply, NewState} |
                                       {stop, Reason, NewState} when Msg :: term(),
                                                                     From :: {pid(), Tag},
                                                                     Tag :: term(),
                                                                     State :: #state{},
                                                                     NewState :: #state{},
                                                                     ...
                                                                     Reply :: term().
handle_call({create,
             {#auth_ret{platform = Platform, accname = AccName}, #acc_create_c2s{}, Client},
             Args},
            _From,
            State) ->
    IP = 
...
handle_call({enter, #auth_ret{accsign = Sign}, EnterMsg, Client}, _From, State) ->
    %% double check
    Ret = case ets:lookup(?MODULE, Sign) of
            [] ->
                %%TODO do enter
                pass;
            [{_, {Client, _Pid}}] ->
...
-spec handle_cast(Msg, State) -> noreply(NewState) | {stop, Reason, NewState} when Msg ::
                                                                                       term(),
                                                                                   Reason :: term(),
                                                                                   State ::
                                                                                       #state{},
...
-spec route_msg(msg(), Args :: proplists:proplist()) -> {ok, Ret :: term()} |
                                                        {error, Reason :: term()}.
...
-spec route_call(msg(), Args :: proplists:proplist()) -> {ok, Ret :: term()} |
                                                         {error, Reason :: term()}.
